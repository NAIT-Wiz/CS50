...
const unsigned int TABLE_SIZE = 50000; // 25 as 26 alphabet characters, could also implement first two letters, or math using all
                                       // letters. Google some good ones.
...
bool load(const char *dictionary)
{
    ...
    // Stores all data in a hash table, put words being with A in the A hash etc. use a hash  function to assign these to the
    // relevant linked list. Hash function, takes a word input, outputs a word corresponding to which bucket to store the word in
    // index [0] will be A. to add new data to hash table, allocate memory for nodes then add data to the node. EG -- node *n =
    // malloc(sizeof(node)); this format copies words into the character array strcpy(n->word, "hello"); to set next pointer n->next
    // = NULL;
